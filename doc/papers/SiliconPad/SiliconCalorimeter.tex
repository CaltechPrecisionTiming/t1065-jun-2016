...
distribution of $\Delta t = t_0-t_1$. We observe a dependence of the time of
flight on the total charge measured in the silicon detector, as shown in
Fig.~\ref{fig:timewalk}. We perform a correction to the time of flight
measurement for each event dependent on the measured charge in the silicon
sensor. The correction is obtained from a second degree polynomial fit to the
distribution of the $\Delta t$ versus total charge collected in the silicon
sensor, as shown in Figure~\ref{fig:timewalk}. We verify that the correction
does flatten the dependence of the time measurement on the integrated charge, as
shown on the right panel of Figure~\ref{fig:timewalk}. An example of the time of
flight distribution after the correction is applied is shown on the right of
Figure~\ref{fig:ChargeDistributionExample} for $32$~GeV electrons after 6
radiation lengths of tungsten. We observe an improvement in the time resolution
...
using a secondary emission calorimeter prototype based on MCP's~\cite{MCPShowerMaxPaper},
