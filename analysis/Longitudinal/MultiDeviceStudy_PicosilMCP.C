...
void DoMultiDeviceStudy( string filename, string outputFilename ) {
  ...
  //Create histogram
  TH1F *histDeltaT = new TH1F("histDeltaT","; Time [ns];Number of Events", 100, 3.2, 4.2); // Weights 50-50
  TH1F *histDeltaTWeighted = new TH1F("histDeltaTWeighted","; Time [ns];Number of Events", 100, 2, 6); //Weights each event based on charge
...
void MultiDeviceStudy_PicosilMCP() {
  ...
  makeTimeResolution("output66_PicosilMCP.root"); // Outputs PDFs with histograms
}
